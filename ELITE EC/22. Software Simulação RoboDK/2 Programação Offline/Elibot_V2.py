...
class RobotPost(object):
    ...
    STR_V = 'V=100MM/S'         # set default cartesian speed
    ...
    def MoveL(self, pose, joints, conf_RLF=None):
        ...
        if pose is not None:
            ...
            joints_ex = ["%.5f"%x for x in joints]
            self.addline('SETJOINT P%03i ' % pid + ','.join(joints_ex))
            self.addline2('MOVL P%03i %s %s ACC=50 DEC=50' % (pid, self.STR_V, self.STR_PL))
    ...
    def MoveC(self, pose1, joints1, pose2, joints2, conf_RLF_1=None, conf_RLF_2=None):
        ...
        if pose1 is not None:
            if self.bStartCir:
                ...
                pid = self.P_COUNT-1
                self.addline2('MOVC P%03i %s %s ACC=50 DEC=50' % (pid, self.STR_V, self.STR_PL))

            pid = self.P_COUNT
            self.P_COUNT = self.P_COUNT + 1
            # joints_ex = [str(x) for x in joints] + ['0.0000', '0.0000']
            joints_ex = ["%.5f"%x for x in joints1]
            self.addline('SETJOINT P%03i ' % pid + ','.join(joints_ex))
            self.addline2('MOVC P%03i %s %s ACC=50 DEC=50' % (pid, self.STR_V, self.STR_PL))

            pid = self.P_COUNT
            self.P_COUNT = self.P_COUNT + 1
            joints_ex = ["%.5f"%x for x in joints2]
            self.addline('SETJOINT P%03i ' % pid + ','.join(joints_ex))
            self.addline2('MOVC P%03i %s %s ACC=50 DEC=50' % (pid, self.STR_V, self.STR_PL))
        ...
        # self.addlog('Circular move not defined')
